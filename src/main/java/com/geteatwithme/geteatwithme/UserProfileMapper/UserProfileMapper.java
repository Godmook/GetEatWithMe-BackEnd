...
public interface UserProfileMapper {
    ...
    //수정 필요
    @Insert("INSERT INTO post VALUES(#{id}, #{restaurant},#{meeting_place},#{category},#{max_people}, #{cur_people}, #{meeting_date}, #{meeting_time}, #{contents}, #{Longtitude}, #{Latitude},(SELECT MAX(post_id)+1 FROM post ALIAS_FOR_SUBQUERY JOIN userprofile WHERE post.id=userprofile.id), #{meet_x},#{meet_y},#{restaurant_id},#{visible},#{post_visible},#{sec})")
    int insertUserPost(@Param("id")String id, @Param("restaurant")String restaurant, @Param("meeting_place")String meeting_place,@Param("category")int category, @Param("max_people")int max_people, @Param("cur_people")int cur_people, @Param("meeting_date")String meeting_date, @Param("meeting_time")String meeting_time, @Param("contents")String contents, @Param("Longtitude")Double Longtitude, @Param("Latitude")Double Latitude,@Param("meet_x")Double meet_x,@Param("meet_y")Double meet_y,@Param("restaurant_id")int restaurant_id,@Param("visible")int visible,@Param("post_visible")int post_visible,@Param("sec")int sec);
    ...
    @Insert("INSERT INTO useralarm VALUES(#{id},#{request},#{opposite_id},#{post_id},#{view},#{nickname},#{oppostie_nickname},(SELECT MAX(alarm_id)+1 FROM useralarm)")
